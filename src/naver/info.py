...
class Info:
    def __init__(self, save_dir, file_name):
        ...
        self.f = Driver().open_file(save_dir=save_dir, file_name=file_name, columns=['id', 'link', 'title', 'author', 'genre', 'description'])
    ...
    def get_webtoon_info(self):
        ...
        for webtoon in tqdm(webtoon_model, desc='webtoon info'):
            ...
            try:
                ...
                webtoon_model.remove(webtoon)
            
        Driver().save_webtoon_csv(webtoon, self.f)
        self.driver.quit()
    
if __name__ == '__main__':
    info = Info('data/webtoon/', 'info.csv')
    info.get_webtoon_list()
